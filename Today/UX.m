...
-(UIAlertView*)makeAlertWithTitle:(NSString*)titleST message:(NSString*) messageST cancelTitle:(NSString*)cancelST target:(id)target {
    UIAlertView *alert = [[UIAlertView alloc]
                           initWithTitle:titleST
                           message:messageST
                           delegate:target
                           cancelButtonTitle:cancelST
...
-(UIView*)setViewForMovieFullscreen {
    ...
    UIView *viewForMovie = [[UIView alloc] initWithFrame:screenRect];
    viewForMovie.backgroundColor = [UIColor blackColor];
    
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        viewForMovie.frame = CGRectMake(0, -170, 768 , 1364);
        [[[[UIApplication sharedApplication] delegate] window] addSubview:viewForMovie];
    } else {
        if (IS_IPHONE_5) {
            viewForMovie.frame = CGRectMake(0, 0, 320 , 568);
            [[[[UIApplication sharedApplication] delegate] window] addSubview:viewForMovie];
        } else {
            viewForMovie.frame = CGRectMake(0, -45, 320 , 568);
            [[[[UIApplication sharedApplication] delegate] window] addSubview:viewForMovie];
        }
    }
    viewForMovie.alpha = 1;
    return viewForMovie;
}
